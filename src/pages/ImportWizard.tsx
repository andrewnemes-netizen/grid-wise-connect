...
const CANONICAL_FIELDS: { key: string; label: string; required?: boolean }[] = [
  ...
  { key: "kw_per_socket", label: "kW per Socket" },
  { key: "proposed_kw", label: "Total Site kW" },
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm">
              ...
              {batch.target_wp_id ? <Link to={`/wp/${batch.target_wp_id}/overview`} className="underline">open WP</Link> : null}.
            </p>
            <div className="flex gap-2 flex-wrap">
              <Button asChild variant="default"><Link to={`/portfolio`}>View in Portfolio</Link></Button>
              {batch.target_wp_id && <Button asChild variant="outline"><Link to={`/wp/${batch.target_wp_id}/overview`}>Open Work Package</Link></Button>}
