 ...
 * NPG Catalog Crawler
 * 
 * Discovers every dataset on the Northern Powergrid Opendatasoft portal
 * using the Explore API v2.1 catalog endpoint.
 * 
 * Endpoints used:
 *   GET /catalog/datasets                      — list all datasets (paginated)
 *   GET /catalog/datasets/{dataset_id}         — detailed schema per dataset
 *   GET /catalog/datasets/{dataset_id}/exports  — available export formats
 * 
 * For each dataset it generates:
 *   - records endpoint URL
 *   - export URLs (CSV, JSON, GeoJSON, Parquet)
 *   - attachment/file URLs
 *   - geometry detection
 *   - primary key guess
 *   - field schema snapshot + hash for drift detection
 */

const BASE_URL = "https://northernpowergrid.opendatasoft.com/api/explore/v2.1";
const PORTAL_BASE = "https://northernpowergrid.opendatasoft.com/explore/dataset";
const DNO_KEY = "NPG";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const apiKey = body.api_key || Deno.env.get("NPG_API_KEY") || null;

    console.log("[crawler] Starting NPG catalog discovery...");

    // Step 1: Paginate through catalog to discover all datasets
    ...
    // Step 2: Process each dataset and upsert into registry
    ...
    for (const ds of allDatasets) {
      try {
        ...
        // Upsert into dno_dataset_registry
...
/**
 * Process a single dataset from the catalog response into a registry entry.
 */
function processDataset(ds: any): Record<string, any> {
  ...
  // Detect available exports from the features
  const exportFormats: string[] = [];
  // Standard Opendatasoft exports: csv, json, geojson, parquet, xlsx, shp
  for (const fmt of ["csv", "json", "geojson", "parquet", "xlsx", "shp"]) {
    exportFormats.push(fmt);
  }
  ...
  // Guess primary key — look for fields named *_id, id, asset_id, etc.
  ...
  // Guess storage table based on geometry type
...
/** Guess primary key from field names */
...
/** Simple string hash for schema drift detection */
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (resp.status === 429) {
        ...
        console.warn(`[crawler] Rate limited, backing off ${Math.round(backoff)}ms (attempt ${attempt + 1})`);
        await sleep(backoff);
        continue;
      }
      ...
      if (!resp.ok && attempt < maxRetries) {
        ...
        console.warn(`[crawler] HTTP ${resp.status} for ${url}, retrying in ${backoff}ms`);
      ...
      if (attempt < maxRetries) {
        const backoff = Math.pow(2, attempt) * 500;
        console.warn(`[crawler] Fetch error for ${url}, retrying in ${backoff}ms:`, err);
        await sleep(backoff);
        continue;
      }
      throw err;
