...
CREATE OR REPLACE FUNCTION public.npg_circuits_for_substation(p_name text)
RETURNS TABLE (
  circuit_id text,
  voltage_kv int,
...
AS $$
  ...
  matched AS (
    SELECT m.*
    FROM public.npg_circuit_monthly m, q
    WHERE q.n <> ''
      AND (
  ...
  latest AS (
    ...
    FROM matched
    ORDER BY circuit_id, year DESC, month DESC
  ),
  peak12 AS (
    SELECT circuit_id, max(peak_mw) AS p12
    FROM matched
    WHERE (year*12 + month) >= (
      SELECT max(year*12 + month) - 11 FROM matched
    )
    GROUP BY circuit_id
  )
  ...
  FROM latest l
  LEFT JOIN peak12 p USING (circuit_id)
  ORDER BY p.p12 DESC NULLS LAST
  LIMIT 20;
$$;

GRANT EXECUTE ON FUNCTION public.npg_circuits_for_substation(text) TO authenticated, anon, service_role;
