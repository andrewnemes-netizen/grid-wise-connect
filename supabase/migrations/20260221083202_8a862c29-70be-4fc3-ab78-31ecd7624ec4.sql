...
DROP FUNCTION IF EXISTS public.get_geo_layer_geojson(uuid, text, text, integer);
  ...
  ELSIF _storage_table = 'geo_points' THEN
    ...
    FROM (
      SELECT jsonb_build_object(
        'type', 'Feature',
        'geometry', ST_AsGeoJSON(
          CASE WHEN _data_srid != 4326 THEN ST_Transform(geom, 4326) ELSE geom END
        )::jsonb,
        'properties', jsonb_build_object(
          'id', id, 'name', name, 'asset_id', asset_id,
          'dno', dno, 'source_date', source_date
