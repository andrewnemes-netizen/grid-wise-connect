...
BEGIN
  ...
  IF array_length(v_next_stages, 1) IS NOT NULL AND array_length(p_next_recipient_user_ids, 1) IS NOT NULL THEN
    FOREACH v_next IN ARRAY v_next_stages LOOP
      -- Waiting-stage seed: currently only poc_offer_awaiting (20 working days).
      -- Extend here when more waiting stages are configured client-side.
      v_is_waiting := v_next = 'poc_offer_awaiting';
      IF v_is_waiting THEN
        -- 20 working days from today, skipping weekends
