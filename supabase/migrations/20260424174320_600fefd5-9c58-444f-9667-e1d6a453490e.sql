DROP FUNCTION IF EXISTS public.find_nearest_compatible_lv_main(double precision, double precision, double precision);

CREATE FUNCTION public.find_nearest_compatible_lv_main(
...
nearby AS (
  ...
  FROM geo_cables c
  CROSS JOIN fp
  WHERE ST_DWithin(c.geom::geography, fp.geom::geography, p_search_m)
...
metric2 AS (
  SELECT
    m.*,
    ST_Distance(ST_Transform(m.snap_pt, 27700), ST_Transform(m.fp_geom, 27700)) AS snap_distance_m
  FROM metric m
),
parsed AS (
  SELECT
...
scored AS (
  SELECT
    f.cable_id, f.asset_id, f.cst AS conducting_section_type, f.feeder_name, f.source_site_name,
  ...
  LEFT JOIN lv_capacity_lookup l
    ON l.family = f.fam AND l.size_value = f.size_val AND l.size_unit = f.size_u
)
SELECT
  s.cable_id, s.asset_id, s.conducting_section_type, s.feeder_name, s.source_site_name,
  s.distance_m, s.calc_score AS score,
  ST_X(s.snap_pt) AS snap_lon, ST_Y(s.snap_pt) AS snap_lat,
  COALESCE(s.direct_kva, 0) AS direct_kva, COALESCE(s.ducted_kva, 0) AS ducted_kva,
