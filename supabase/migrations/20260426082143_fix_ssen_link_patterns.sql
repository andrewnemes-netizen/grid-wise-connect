...
BEGIN
  IF p_dno = 'SSEN' THEN
    v_rules := '[
      ...
      {"slug":"ssen-dx-primary-supply-areas","display_name":"SSEN DX - Primary Substation Supply Areas","category":"Boundaries","subcategory":"Primary Supply Areas","geometry_type":"Polygon","storage_table":"geo_polygons","match_patterns":["%primary-substation-boundaries%","%primary substation%supply area%"],"exclude_patterns":[]},
      {"slug":"ssen-dx-secondary-supply-areas","display_name":"SSEN DX - Secondary Substation Supply Areas","category":"Boundaries","subcategory":"Secondary Supply Areas","geometry_type":"Polygon","storage_table":"geo_polygons","match_patterns":["%secondary-substation-esa%","%secondary substation%supply area%"],"exclude_patterns":[]},
      {"slug":"ssen-dx-gsp-bsp-supply-areas","display_name":"SSEN DX - GSP & BSP Supply Areas","category":"Boundaries","subcategory":"GSP/BSP","geometry_type":"Polygon","storage_table":"geo_polygons","match_patterns":["%grid-supply-point-gsp-bulk-supply-point%","%gsp%bsp%supply area%","%bulk supply point%"],"exclude_patterns":[]},
      ...
      {"slug":"ssen-dx-smart-meter-lv-feeder","display_name":"SSEN DX - Smart Meter LV Feeder Usage","category":"Performance","subcategory":"Smart Meter","geometry_type":"Point","storage_table":"geo_points","match_patterns":["%smart_meter_prod_lv_feeder%","%smart meter%lv feeder%"],"exclude_patterns":[]},
  ...
  LOOP
    ...
    FOR v_dataset IN
      SELECT id, dataset_id, title FROM dno_dataset_registry
      WHERE dno = p_dno AND active = true
