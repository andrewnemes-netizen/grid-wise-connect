...
CREATE OR REPLACE VIEW public.v_wp_commercial_position AS
WITH
-- Estimates v2: current + approved/awarded/accepted, per WP
awarded_v2 AS (
...
-- Estimate v1 (work_package_estimates): latest APPROVED version per WP
awarded_v1_ranked AS (
  SELECT wpe.work_package_id,
         wpe.total_cost, wpe.total_price,
...
-- Site estimates: latest APPROVED version per site, rolled up to WP via wp_sites
awarded_site_ranked AS (
  SELECT ws.work_package_id, se.site_id,
         se.total_cost, se.total_price,
