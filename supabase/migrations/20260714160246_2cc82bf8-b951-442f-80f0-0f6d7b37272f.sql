...
BEGIN
  ...
  VALUES (v_uid, 'approve', 'site_estimate', p_estimate_id,
          jsonb_build_object('version_number', v_row.version_number, 'total_price', v_row.total_price));

  RETURN v_row;
END;
$$;

REVOKE ALL ON FUNCTION public.approve_site_estimate(UUID, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.approve_site_estimate(UUID, TEXT) TO authenticated, service_role;

-- =========================
-- SITE ESTIMATE: CLONE TO NEW DRAFT
-- =========================
  ...
  IF v_src.status = 'APPROVED' THEN
    UPDATE public.site_estimates
       SET status = 'SUPERSEDED',
           superseded_by_estimate_id = v_new.id,
           updated_at = now()
     WHERE id = v_src.id;
  END IF;

  INSERT INTO public.audit_log (actor_id, action, entity_type, entity_id, metadata)
  VALUES (v_uid, 'clone_to_draft', 'site_estimate', v_new.id,
          jsonb_build_object('source_id', v_src.id, 'version_number', v_next));

  RETURN v_new;
END;
$$;

REVOKE ALL ON FUNCTION public.clone_site_estimate_to_draft(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.clone_site_estimate_to_draft(UUID) TO authenticated, service_role;

-- =========================
-- WP ESTIMATE: RECALC TOTALS
-- =========================
CREATE OR REPLACE FUNCTION public.recalculate_wp_estimate_totals(p_estimate_id UUID)
...
DECLARE
  ...
  v_sites_cost NUMERIC(14,2);
  v_sites_price NUMERIC(14,2);
  v_adj_cost NUMERIC(14,2);
  v_adj_price NUMERIC(14,2);
BEGIN
  IF v_uid IS NULL OR NOT public.is_gridwise_staff(v_uid) THEN
    RAISE EXCEPTION 'Not authorised';
  END IF;
  ...
  IF v_row.status = 'APPROVED' THEN
    RAISE EXCEPTION 'Cannot recalculate an APPROVED WP estimate';
  END IF;

  SELECT COALESCE(SUM(contribution_cost),0), COALESCE(SUM(contribution_price),0)
    INTO v_sites_cost, v_sites_price
    FROM public.wp_estimate_sites
   WHERE wp_estimate_id = p_estimate_id AND included = true;

  SELECT COALESCE(SUM(amount_cost),0), COALESCE(SUM(amount_price),0)
    INTO v_adj_cost, v_adj_price
    FROM public.wp_estimate_adjustments
   WHERE wp_estimate_id = p_estimate_id;

  UPDATE public.work_package_estimates
     SET sites_total_cost = v_sites_cost,
         sites_total_price = v_sites_price,
         adjustments_total_cost = v_adj_cost,
         adjustments_total_price = v_adj_price,
         total_cost = v_sites_cost + v_adj_cost,
         total_price = v_sites_price + v_adj_price,
         total_markup = (v_sites_price + v_adj_price) - (v_sites_cost + v_adj_cost),
         updated_at = now()
   WHERE id = p_estimate_id
   RETURNING * INTO v_row;
...
REVOKE ALL ON FUNCTION public.recalculate_wp_estimate_totals(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.recalculate_wp_estimate_totals(UUID) TO authenticated, service_role;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_unapproved
   ...
   WHERE wes.wp_estimate_id = p_estimate_id
     AND wes.included = true
  ...
  -- Ensure totals are up to date before locking
  ...
  INSERT INTO public.audit_log (actor_id, action, entity_type, entity_id, metadata)
  VALUES (v_uid, 'approve', 'wp_estimate', p_estimate_id,
          jsonb_build_object('version_number', v_row.version_number, 'total_price', v_row.total_price));
...
REVOKE ALL ON FUNCTION public.approve_wp_estimate(UUID, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.approve_wp_estimate(UUID, TEXT) TO authenticated, service_role;

-- =========================
-- WP ESTIMATE: CLONE TO NEW DRAFT
-- =========================
...
BEGIN
  ...
  INSERT INTO public.audit_log (actor_id, action, entity_type, entity_id, metadata)
  VALUES (v_uid, 'clone_to_draft', 'wp_estimate', v_new.id,
          jsonb_build_object('source_id', v_src.id, 'version_number', v_next));
...
REVOKE ALL ON FUNCTION public.clone_wp_estimate_to_draft(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.clone_wp_estimate_to_draft(UUID) TO authenticated, service_role;
