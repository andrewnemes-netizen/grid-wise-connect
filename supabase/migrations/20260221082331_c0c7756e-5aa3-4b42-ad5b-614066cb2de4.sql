...
-- Drop the old 5-param overload that returns json (wrong type)
DROP FUNCTION IF EXISTS public.get_geo_layer_geojson(uuid, text, text, integer, text);

-- Recreate with jsonb return + aggressive simplification for wide viewports
...
BEGIN
  ...
  IF _storage_table = 'geo_substations' THEN
    SELECT coalesce(jsonb_agg(f), '[]'::jsonb) INTO _result
    FROM (
      SELECT jsonb_build_object(
        'type', 'Feature',
        'geometry', ST_AsGeoJSON(
          CASE WHEN _simplify_tolerance > 0
            THEN ST_Simplify(ST_Transform(geom, 4326), _simplify_tolerance)
        ...
        ) || COALESCE(attrs_json, '{}'::jsonb)
      ) AS f
      FROM geo_substations
      WHERE layer_id = _layer_id
        AND geom IS NOT NULL
        AND (_bbox_geom IS NULL OR ST_Intersects(geom, ST_Transform(_bbox_geom, 27700)))
