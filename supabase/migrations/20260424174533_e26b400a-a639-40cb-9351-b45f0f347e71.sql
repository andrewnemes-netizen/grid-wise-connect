...
RETURNS TABLE(
  ...
  snap_lon double precision, snap_lat double precision, direct_kva numeric, ducted_kva numeric,
...
WITH fp AS (
  SELECT ST_SetSRID(ST_MakePoint(p_lon, p_lat), 4326) AS geom,
         -- Convert metres to a degree buffer (1 deg lat ~111km; lon scaled by cos(lat))
         (p_search_m / 111000.0) * 1.5 AS deg_buf
),
nearby AS (
  -- Use && bbox operator + ST_DWithin in degrees for index usage
  SELECT
    ...
    c.geom AS cable_geom, fp.geom AS fp_geom
  FROM geo_cables c CROSS JOIN fp
  WHERE c.geom && ST_Expand(fp.geom, fp.deg_buf)
    AND ST_DWithin(c.geom, fp.geom, fp.deg_buf)
    AND UPPER(COALESCE(c.attrs_json->>'conducting_section_type', c.attrs_json->>'CONDUCTING_SECTION_TYPE', '')) LIKE 'LV%'
),
metric AS (
  SELECT n.*,
    ST_Distance(ST_Transform(n.cable_geom, 27700), ST_Transform(n.fp_geom, 27700)) AS distance_m,
    ST_ClosestPoint(n.cable_geom, n.fp_geom) AS snap_pt
  FROM nearby n
),
metric2 AS (
  SELECT m.*, ST_Distance(ST_Transform(m.snap_pt, 27700), ST_Transform(m.fp_geom, 27700)) AS snap_distance_m
  FROM metric m WHERE m.distance_m <= p_search_m
),
parsed AS (
  SELECT n.*,
    CASE
  ...
  FROM metric2 n
  WHERE n.snap_distance_m <= CASE WHEN p_search_m <= 25 THEN 15 WHEN p_search_m <= 50 THEN 30 ELSE 50 END
),
family_mapped AS (
  SELECT p.*,
...
scored AS (
  SELECT f.cable_id, f.asset_id, f.cst AS conducting_section_type, f.feeder_name, f.source_site_name,
    f.distance_m, f.snap_distance_m, f.snap_pt,
    ...
    (CASE WHEN COALESCE(l.ev_compatible_55kva_80a, false) THEN 1000 ELSE 0 END
     ...
     - (f.snap_distance_m * 5)) AS calc_score
...
SELECT s.cable_id, s.asset_id, s.conducting_section_type, s.feeder_name, s.source_site_name,
  ...
  ST_X(s.snap_pt) AS snap_lon, ST_Y(s.snap_pt) AS snap_lat,
