ALTER TYPE public.site_stage_key ADD VALUE IF NOT EXISTS 'issue_survey_design_quote';

INSERT INTO public.stage_definitions (org_id, key, label, category, requires_owner, allowed_owner_roles)
VALUES (NULL, 'issue_survey_design_quote', 'Issue Survey / Design Quote', 'pre-con', true, ARRAY['admin','engineer','consultant'])
ON CONFLICT (org_id, key) DO UPDATE SET label = EXCLUDED.label;
...
BEGIN
  ...
  IF array_length(v_next_stages, 1) IS NOT NULL AND array_length(p_next_recipient_user_ids, 1) IS NOT NULL THEN
    FOREACH v_next IN ARRAY v_next_stages LOOP
      -- Waiting-stage seed: poc_offer_awaiting counts down 20 working days;
      -- survey_po_gate counts up from quote-issued date with a 10-working-day SLA target.
      v_is_waiting := v_next IN ('poc_offer_awaiting', 'survey_po_gate');
      IF v_is_waiting THEN
        ...
        ORDER BY d
        OFFSET (CASE WHEN v_next = 'survey_po_gate' THEN 9 ELSE 19 END) LIMIT 1;
      ELSE
        v_wait_target := NULL;
      END IF;
      ...
      FOREACH v_site IN ARRAY p_site_ids LOOP
        ...
        VALUES
          (p_wp_id, v_site, v_next::public.site_stage_key, 'in_progress'::public.site_stage_state,
           ...
           CASE WHEN v_is_waiting THEN now() ELSE NULL END,
           v_wait_target,
           NULL, NULL)
        ON CONFLICT (site_id, stage) DO UPDATE
          SET work_package_id = EXCLUDED.work_package_id,
              ...
              wait_started_at = CASE WHEN v_is_waiting AND site_stage_status.wait_started_at IS NULL
                                     THEN now() ELSE site_stage_status.wait_started_at END,
              wait_target_date = CASE WHEN v_is_waiting AND site_stage_status.wait_target_date IS NULL
