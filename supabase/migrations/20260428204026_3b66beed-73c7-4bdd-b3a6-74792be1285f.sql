...
SET linked_layer_id = l.id,
    ...
    last_sync_error = 'No ingestable source resource available for this catalogue entry',
    updated_at = now()
FROM public.layer_registry l
WHERE d.dno = 'SSEN'
  AND d.dataset_id = 'dx-gis_network_line_data'
  AND l.slug = 'ssen-dx-gis-network-lines';

UPDATE public.dno_dataset_registry
SET endpoint_records = NULL,
    last_sync_status = 'never',
    last_sync_error = NULL,
    last_sync_rows = 0,
    updated_at = now()
WHERE dno = 'SSEN'
  AND dataset_id LIKE 'dx-%'
  AND endpoint_records IS NOT NULL
  AND endpoint_export_csv IS NOT NULL
  AND endpoint_records = endpoint_export_csv
  AND last_sync_status = 'error'
  AND last_sync_error LIKE 'SyntaxError:%is not valid JSON%';
...
WHERE dno = 'SSEN'
  AND dataset_id LIKE 'dx-%'
  AND COALESCE(endpoint_export_geojson, endpoint_export_csv, endpoint_records) IS NULL
  AND last_sync_status = 'error';